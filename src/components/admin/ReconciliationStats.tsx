...
export function ReconciliationStats({ isLoading = false }: ReconciliationStatsProps) {
  ...
  const reconciliationRate = data
    ? Math.round((data.stats.reconciledPayments / data.stats.totalPayments) * 100) || 0
    : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-700">Pending Reconciliation</h3>
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          ...
          <p className="text-2xl font-bold text-gray-900">{reconciliationRate}%</p>
          <p className="text-sm text-gray-600 mt-2">
            {data.stats.reconciledPayments} of {data.stats.totalPayments} payments
          </p>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          ...
          <p className="text-sm text-gray-600 mt-2">Average reconciliation time</p>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="text-center p-4 bg-green-50 rounded-lg border border-green-200">
            ...
            <p className="text-2xl font-bold text-green-900">{data.stats.reconciledPayments}</p>
          ...
          <div className="text-center p-4 bg-blue-50 rounded-lg border border-blue-200">
            ...
            <p className="text-2xl font-bold text-blue-900">{data.stats.receivedPayments}</p>
          ...
          <div className="text-center p-4 bg-yellow-50 rounded-lg border border-yellow-200">
            ...
            <p className="text-2xl font-bold text-yellow-900">{data.stats.pendingPayments}</p>
          ...
          <div className="text-center p-4 bg-red-50 rounded-lg border border-red-200">
            ...
            <p className="text-2xl font-bold text-red-900">{data.stats.rejectedPayments}</p>
            <p className="text-xs text-red-700 mt-1">Rejected</p>
          </div>

          {/* Total */}
          <div className="text-center p-4 bg-gray-50 rounded-lg border border-gray-200">
            <div className="w-6 h-6 text-gray-600 mx-auto mb-2 font-bold text-lg">∑</div>
      ...
      {/* Health Indicator */}
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Reconciliation Health</h3>
        <div className="space-y-4">
          {/* Progress Bar */}
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-700">Reconciliation Progress</span>
              <span className="text-sm font-semibold text-gray-900">{reconciliationRate}%</span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-700">
                {reconciliationRate >= 80 ? 'Excellent' : reconciliationRate >= 60 ? 'Good' : 'Needs Attention'}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 bg-blue-600 rounded-full"></div>
              <span className="text-sm text-gray-700">
                {data.stats.averageReconciliationTime <= 24 ? 'Fast Processing' : 'Standard Processing'}
              </span>
            </div>
          </div>
        </div>
